...
import com.graphhopper.routing.weighting.QueryWeighting;
...
public class QueryGraphTest {
    ...
    public void testTurnCostsProperlyPropagated_Issue282() {
        ...
        FastestWeighting weighting = new FastestWeighting(encoder, new DefaultTurnCostProvider(encoder, graphWithTurnCosts.getTurnCostStorage()));

        // no turn costs initially
        assertEquals(0, weighting.calcTurnWeight(edge0.getEdge(), 1, edge1.getEdge()), .1);

        // now use turn costs with base graph
        ...
        Weighting queryWeighting = new QueryWeighting(weighting, new DefaultTurnCostProvider(weighting.getFlagEncoder(), qGraph.getTurnCostStorage()));
        ...
        assertEquals(10, queryWeighting.calcTurnWeight(fromQueryEdge, 1, toQueryEdge), .1);
